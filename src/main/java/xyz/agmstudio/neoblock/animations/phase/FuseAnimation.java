...
public class FuseAnimation extends UpgradePhaseAnimation {
    ...
    @Override public void tick(ServerLevel level, LevelAccessor access) {
        ...
        while (iterator.hasNext()) {
            ...
            if (tick == 0) level.playSound(null, NeoBlock.POS, SoundEvents.CREEPER_PRIMED, SoundSource.BLOCKS, volume, 1.0f);

            for (int i = 0; i < 8; i++) {
                Vec3 particlePos = Vec3.atCenterOf(NeoBlock.POS).add(
            ...
            if (tick % 5 == 0) {
                Vec3 sparkPos = Vec3.atCenterOf(NeoBlock.POS).add(
