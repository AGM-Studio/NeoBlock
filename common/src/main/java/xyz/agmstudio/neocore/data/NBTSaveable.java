package xyz.agmstudio.neocore.data;
...
public interface NBTSaveable {
    ...
    default void load(CompoundTag tag) {
        ...
        try {
                ...
                if (field.isAnnotationPresent(NBTData.class)) {
                    ...
                    if (type.isEnum()) {
                        try {
                            ...
                            NeoMod.CORE_LOGGER.error("Failed to load enum field: {}", field.getName(), e);
            ...
            NeoMod.CORE_LOGGER.error("NBT loading into {} has been aborted by {}", clazz.getSimpleName(), e);
        } catch (Exception e) {
            NeoMod.CORE_LOGGER.error("Failed to load NBT into {}", clazz.getSimpleName(), e);
    ...
    static <R> R getFromTag(CompoundTag tag, String key, Class<R> type, Object root, boolean nullable) {
        ...
        if (NBTSaveable.class.isAssignableFrom(type)) {
            ...
            NeoMod.CORE_LOGGER.error("Unsupported tag type: {}", type);
            return null;
        }
    }
